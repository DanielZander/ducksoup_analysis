...
# %%
data_name = "mkUltimatum_Game_Conflict1"
...
for subject in raw_data['participant.id_in_session']:
    ...
    for round in range(1, 11):
        ...
        if subject_row[f'ultimatum_game_conflict.{round}.player.player_role'].values[0] == "proposer":
            rt = subject_row[f'ultimatum_game_conflict.{round}.player.proposer_response_time_ms'].values[0]
        else:
            rt = subject_row[f'ultimatum_game_conflict.{round}.player.responder_response_time_ms'].values[0]
            
        row_data = {
            'participant_code': subject_row['participant.code'].values[0],
            'session_code': subject_row['session.code'].values[0],
            'prolific_id': subject_row['ultimatum_game_conflict.10.player.prolific_id'].values[0],
            'sid': subject_row['session.config.name'].values[0],
            'mk_session': subject_row['session.config.id'].values[0],
            'trial_payoff': subject_row[f'ultimatum_game_conflict.{round}.player.trial_payoff'].values[0],
            'round_nb': subject_row[f'ultimatum_game_conflict.{round}.subsession.round_number'].values[0],  
            'role': subject_row[f'ultimatum_game_conflict.{round}.player.player_role'].values[0],
            'player': subject_row['participant.id_in_session'].values[0],
            'dyad': subject_row[f'ultimatum_game_conflict.{round}.player.dyad'].values[0],
            'manipulation': subject_row[f'ultimatum_game_conflict.{round}.player.manipulation'].values[0],
            'responded': subject_row[f'ultimatum_game_conflict.{round}.player.responded'].values[0],
            'sent_amount': subject_row[f'ultimatum_game_conflict.{round}.group.sent_amount'].values[0],
            'offer_response': subject_row[f'ultimatum_game_conflict.{round}.group.responder_accepted'].values[0],
            'rt': rt,
            'mean_social_dominance': mean([float(subject_row[f'ultimatum_game_conflict.10.player.social_dominance_{question_social}'].values[0]) for question_social in range(1, 9)]),
            'mean_aggresive_dominance': mean([float(subject_row[f'ultimatum_game_conflict.10.player.aggressive_dominance_{question_aggresive}'].values[0]) for question_aggresive in range(1, 8)])
            }   
    ...
    row_data_debrief = {
        ...
        'mean_social_dominance': mean([float(subject_row[f'ultimatum_game_conflict.10.player.social_dominance_{question_social}'].values[0]) for question_social in range(1, 9)]),
        'mean_aggresive_dominance': mean([float(subject_row[f'ultimatum_game_conflict.10.player.aggressive_dominance_{question_aggresive}'].values[0]) for question_aggresive in range(1, 8)]),
        'sound_quality': subject_row['ultimatum_game_conflict.10.player.final_quality'].values[0],
        'sound_comment': subject_row['ultimatum_game_conflict.10.player.final_quality_comment'].values[0],
        'fidelity':  subject_row['ultimatum_game_conflict.10.player.final_conversation_fidelity'].values[0],
        'fidelity_comment': subject_row['ultimatum_game_conflict.10.player.final_conversation_fidelity_comment'].values[0],
        'xp_goal': subject_row['ultimatum_game_conflict.10.player.final_xp_goal'].values[0],
        'enough_time': subject_row['ultimatum_game_conflict.10.player.enough_time'].values[0],
        'manipulation': subject_row['ultimatum_game_conflict.10.player.manip_yes_no'].values[0],
        'detection_degree': subject_row['ultimatum_game_conflict.10.player.detection_degree'].values[0],
        'manipulation_comment': subject_row['ultimatum_game_conflict.10.player.final_manipulation_comment'].values[0],
        'unique_interactions': subject_row['ultimatum_game_conflict.10.player.unique_interactions'].values[0]
